...
# Good code defaults. For example, it uses "packages and modules":http://flask.pocoo.org/docs/patterns/packages/ by default so that controllers (and models) are split into logical groups in separate files.
# New Project script - sets up everything for you, including generating the secret key (for secure cookies), etc.
