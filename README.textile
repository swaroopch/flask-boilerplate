...
I wanted a super-quick way to create new websites with all my favorite code settings and customizations. So I created this boilerplate project based on the "Flask (Python) microwebframework":http://flask.pocoo.org

Why Flask-Boilerplate? Because you get:

# Good code defaults. For example, (1) it uses "packages and modules":http://flask.pocoo.org/docs/patterns/packages/ by default so that controllers (and models) are split into logical groups in separate files, (2) it merges and compresses your various CSS files into one, your multiple JS files into one, for performance reasons.
# New Project script - sets up everything for you right from including generating the secret key (for secure cookies) to integrating memcache (for example, mimicing @Rails.cache.fetch@ with helpers).
